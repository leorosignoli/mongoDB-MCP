...
export async function listDatabases(connection) {
    try {
        ...
        const metadata = {
            ...
            totalSize: databases.reduce((sum, db) => sum + db.sizeOnDisk, 0),
...
export async function getDatabaseStats(connection, args) {
    try {
        await connection.ensureConnection();
...
export async function getCollectionStats(connection, args) {
    try {
        await connection.ensureConnection();
        const databaseName = sanitizeDatabaseName(args.database);
        if (!args.collection) {
...
export async function getServerStatus(connection) {
    try {
        await connection.ensureConnection();
        const client = connection.getClient();
        const adminDb = client.db('admin');
