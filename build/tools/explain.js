import { sanitizeQuery, sanitizeCollectionName, sanitizeDatabaseName, } from '../mongodb/safety.js';
export async function explainQuery(connection, args) {
    try {
...
export async function explainAggregation(connection, args) {
    try {
        await connection.ensureConnection();
        const databaseName = sanitizeDatabaseName(args.database);
        const collectionName = sanitizeCollectionName(args.collection);
...
function formatExecutionStage(stage) {
    if (!stage)
        return null;
    return {
        stage: stage.stage,
