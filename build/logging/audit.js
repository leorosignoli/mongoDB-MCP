export class AuditLogger {
    ...
    logToolExecution(event) {
        ...
        this.logger.audit(`${auditEvent.action} on ${auditEvent.resource.type}`, context);
    }
    ...
    logPerformanceEvent(operationId, toolName, duration, metadata) {
        const context = {
            operationId,
            toolName,
            duration,
        ...
        this.logger.performance(`Operation completed in ${duration}ms${metadata.slow ? ' (SLOW)' : ''}`, { ...context, duration });
    }
    getEvents(filter) {
        ...
        if (filter?.since) {
            filteredEvents = filteredEvents.filter(e => new Date(e.metadata.timestamp) >= filter.since);
        }
        filteredEvents = filteredEvents.sort((a, b) => new Date(b.metadata.timestamp).getTime() - new Date(a.metadata.timestamp).getTime());
        if (filter?.limit) {
